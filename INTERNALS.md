...
- `weights.len() <= u32::MAX` (debug-asserted).
- All weights are finite and non-negative (debug-asserted).
- `Σ weights > 0` (debug-asserted).

### 2.4. Panic behavior

Empty `weights` always panics. Other precondition violations panic in
debug builds and produce undefined-but-memory-safe output in release.
The "memory-safe" guarantee rests on Lemma 3 (§5.2).
...
Note that *internal* libm SIMD use (using SIMD instructions to
compute one scalar `powf` more quickly) is unaffected by
`black_box`. The 9 ns/call we measure on x86 is the genuine cost
of one scalar `f32::powf` on the host's libm.
...
not blocking publication of `ltsis` itself.
...
- Carpenter, J., Clifford, P. & Fearnhead, P. (1999). An improved
  particle filter for non-linear problems. *IEE Proceedings —
  Radar, Sonar and Navigation*, 146(1), 2–7. Spacings construction
  in the SIR context.
